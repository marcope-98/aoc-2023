...
static std::unordered_map<char, std::size_t> values;

static void set_values(const std::string &ascending_values)
{
  for (std::size_t i = 0; i < ascending_values.size(); ++i)
    values[ascending_values[i]] = i;
}
static std::size_t get_value(const char &character) { return values[character]; }
